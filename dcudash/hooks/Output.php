...
class dcudash_hook_Output extends _HOOK_CLASS_
{
	/**
	...
	{
		if ( ! isset( \IPS\Settings::i()->dcudash_error_dash ) or ! \IPS\Settings::i()->dcudash_error_dash )
		{
			parent::error( $message, $code, $httpStatusCode, $adminMessage, $httpHeaders, $extra, $faultyAppOrHookId );
		}

		/* When we log out, the user is taken back to the dash they were just on. If this is producing a "no permission" error, redirect them to the index instead */
		if ( isset( \IPS\Request::i()->_fromLogout ) )
		{
			...
			// would happen if guests cannot view the index dash) we need to change _fromLogout, but we can't unset it because _fromLogout={anything}
		...
		/* Send output */
		\IPS\dcudash\Dashes\Dash::errorDash( $title, $message, $code, $httpStatusCode, $httpHeaders );
	}
