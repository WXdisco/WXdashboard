...
class dcudash_hook_Lang extends _HOOK_CLASS_
{
	/**
	 * Set words
	 *
	...
	public function languageInit()
	{
		...
		/* Ensure applications set up correctly before task is executed. Dashes, for example, needs to set up spl autoloaders first */
		\IPS\Application::applications();
		
		/* Add in the database specific language bits and bobs */
		foreach( \IPS\dcudash\Databases::getDatabaseDataFromStore() as $database )
		{
			...
			$fieldsClass = '\IPS\dcudash\Fields' . $database['database_id'];
